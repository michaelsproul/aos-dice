...
data Outcome = CastLived | CastDied | Failed | MiscastDied | MiscastLived
...
runSimulation castingValue numDice wounds = do
    rolls <- mapM (\_ -> d6) [0..numDice]
    let ones = length (filter (==1) rolls)
    if ones >= 2 then do
        -- Miscast
        d3wounds <- d3
        let woundsSuffered = d3wounds
        ...
        -- Cast
        damageRolls <- mapM (\_ -> d6) [0..4]
        let ones = length (filter (==1) damageRolls)
        selfDamage <- mapM (\_ -> d3) [0..ones]
        if selfDamage >= wounds then
            return CastDied
        else
            return CastLived
    else
